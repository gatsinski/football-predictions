...
def main(argv):
    ...
    with open('training-results.csv', 'w') as stream:
        csvwriter = csv.writer(stream)
        csvwriter.writerow(['Run number', 'Accuracy', 'Loss'])
        for i in range(0, 10):
            model.train(input_fn=train_input_fn, steps=100)
            evaluation_result = model.evaluate(input_fn=test_input_fn)
            csvwriter.writerow([int(i + 1), evaluation_result['accuracy'], evaluation_result['average_loss']])
